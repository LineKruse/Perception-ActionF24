...
Take a moment to think what these “results” could mean if this was a proper experiment which was not run by your teacher. How would you interpret this? Does this match your hypothesis? Especially look at the MAD (maximum absolute deviation from the direct path), the AUC (area under the curve) and the reaction time. 
...
### Try to plot the distribution of one or more of the trajectory measures (e.g. AUC or MAD) and check for bimodality 
TIP! You can check for bimodality in two ways. 
1) - Extract the trial-wise AUC/MAD scores from the "measures" in your mt object. Plot their distribution and use the "bimodality_coefficient" function to compute bimodality coefficient. 
2) - Use the "mt_check_bimodality" function, which will automatically comptue the bimodality coefficient from an inputted mt object. 

```{r}
#Manually extract AUC scores and compute bimodality 
measures_auc <- m5$measures
ggplot(measures_auc, aes(AUC))+
  geom_density()
hist(measures_auc$AUC)

bimodality_coefficient(measures_auc$AUC, na.rm = FALSE)

#Use the mt_check_bimodality 
x <- mt_check_bimodality(
  m5,
  use = "measures",
  use_variables = "AUC"
)
